...
public class GestorDatos {
    ...
    //metodo para leer archivo de usuarios y validar que el usuario y contraseña ingresados existan en el archivo
    public static boolean leerArchivoUsuarios(String usuario, String contraseña) {
        try (BufferedReader br = new BufferedReader(new FileReader("datosUsuarios.txt"))) {
            String linea;
            while ((linea = br.readLine()) != null) {
                String[] datos = linea.split(",");
                String usuarioArchivo = datos[5];
                String contraseñaArchivo = datos[4];
                if (usuario.equals(usuarioArchivo) && contraseña.equals(contraseñaArchivo)) {
                    return false;
                }
            }
        } catch (FileNotFoundException e) {
            System.out.println("No se encontró el archivo de datos de usuarios");
        } catch (IOException e) {
            System.err.println("Error al leer el archivo de datos de usuarios: " + e.getMessage());
        }
        return true;
